...
function renderTextToData(text: string, fontSize = 200, fontFamily = 'sans-serif', fontWeight = 'bold') { 
...
function processImage(img: HTMLImageElement | ImageData): ImageData { 
    const MAX_SIZE = 1200; 
    let width = (img as HTMLImageElement).naturalWidth || (img as ImageData).width; 
    let height = (img as HTMLImageElement).naturalHeight || (img as ImageData).height; 
...
export default function MetallicPaint({ 
    ...
    const uploadTexture = useCallback((imgData: ImageData, dpr: number = 1) => { 
        ...
        canvas.style.width = `${imgData.width / dpr}px`; 
        canvas.style.height = `${imgData.height / dpr}px`; 
    ...
    useEffect(() => { 
        ...
        if (text) { 
            ...
            if (textData) { 
                const processed = processImage(textData); 
                uploadTexture(processed, dpr); 
                setTextureReady(true); 
            } 
        } 
    }, [ready, text, fontSize, fontFamily, fontWeight, uploadTexture]); 
    ...
    useEffect(() => { 
        ...
        const setU = (name: string, val: number) => u[name] && gl.uniform1f(u[name], val); 
        setU('u_seed', seed); 
        setU('u_scale', scale); 
        setU('u_refract', refraction); 
        setU('u_blur', blur); 
        setU('u_liquid', liquid); 
        setU('u_bright', brightness); 
        setU('u_contrast', contrast); 
        setU('u_angle', angle); 
        setU('u_fresnel', fresnel); 
        setU('u_sharp', patternSharpness); 
        setU('u_wave', waveAmplitude); 
        setU('u_noise', noiseScale); 
        setU('u_chroma', chromaticSpread); 
        setU('u_distort', distortion); 
        setU('u_contour', contour); 
    ...
    useEffect(() => { 
        ...
        rafRef.current = requestAnimationFrame(render); 
        return () => rafRef.current ? cancelAnimationFrame(rafRef.current) : undefined; 
    }, [ready, textureReady]); 
